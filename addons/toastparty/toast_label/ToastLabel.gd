...
func move_to(index: int) -> void:
	...
	if index == 0:
		...
		(
			_tween_in
			. tween_property(self, "position", Vector2(position.x, _y), .3 / Settings.game_speed)
			. set_trans(Tween.TRANS_LINEAR)
			. set_ease(Tween.EASE_IN)
			. set_delay(delayed / Settings.game_speed)
		)
		_tween_in.play()
	else:
		_tween_in = get_tree().create_tween()
...
func _tween_destroy_label_timer():
	...
	tween_alpha.tween_property(self, "modulate:a", 0, 0).set_delay(timer_to_destroy)
