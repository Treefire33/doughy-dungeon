...
@export var hover_tooltip: TooltipTrigger;
...
func load_enemy(enemy_data: EnemyData):
    ...
    hover_area.mouse_entered.connect(_update_hover.bind(true));
    hover_area.mouse_exited.connect(_update_hover.bind(false));
    
var target_selection_ais = {
    "Player": func(player: Player, _other_enemies: Array[Entity]):
        return player;,
    "Random": func(player: Player, other_enemies: Array[Entity]):
        var target = null;
        while (target == null || target == self):
            target = ([player] + other_enemies).pick_random();
        return target,
    "Target": func(player: Player, other_enemies: Array[Entity]): # Exclusive to Anemone, who targets Jellyfish
...
func get_target(player: Player, other_enemies: Array[Entity]) -> Entity:
